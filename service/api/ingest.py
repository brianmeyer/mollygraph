...
async def delete_entity_endpoint(
    ...
    rec = graph.get_entity_delete_summary(name)

    if not rec or rec["entity_id"] is None:
        raise HTTPException(status_code=404, detail=f"Entity '{name}' not found")

    entity_id: str = str(rec["entity_id"])
    rels_removed: int = int(rec.get("relationship_count") or 0)
...
async def delete_relationship_endpoint(
    ...
    if rel_type:
        ...
        deleted_count = graph.delete_relationships_between(source, target, rel_type_validated)
    else:
        deleted_count = graph.delete_relationships_between(source, target)
...
async def prune_entities_endpoint(
    ...
    if req.names:
        ...
        names_to_prune = graph.list_orphan_entity_names()
    else:
        raise HTTPException(
            status_code=422,
...
async def list_entities(
    ...
    graph = get_graph_instance()
    try:
        rows, total = graph.list_entities_page(limit=limit, offset=offset, entity_type=type)
